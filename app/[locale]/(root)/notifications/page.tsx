...
import { CheckCircle, Trash2, Loader2, AlertCircle, Clock, Search, Filter } from "lucide-react";
...
type FilterType = 'all' | 'unread' | 'read' | 'trade' | 'news' | 'system' | 'alert' | 'signal';

export default function NotificationsPage() {
  const { user } = useUser();
  const userId = user?.id || "";
  
  const { notification: notifications, loading, markAsRead, markAllAsRead, clearNotifications } = useNotification(userId);
  ...
  const getFilteredNotifications = () => {
    ...
    // Apply type filter
    if (activeFilter === "unread") filtered = filtered.filter(n => !n.is_read);
    else if (activeFilter === "read") filtered = filtered.filter(n => n.is_read);
  ...
  const getNotificationTypeColor = (type: string) => {
    switch (type) {
      case 'trade': return 'bg-emerald-900/20 text-emerald-400';
      case 'news': return 'bg-blue-900/20 text-blue-400';
      case 'system': return 'bg-purple-900/20 text-purple-400';
      case 'alert': return 'bg-amber-900/20 text-amber-400';
      case 'signal': return 'bg-cyan-900/20 text-cyan-400';
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-6 flex flex-col gap-4 rounded-lg border border-slate-700/50 bg-slate-800/30 p-4 backdrop-blur-sm md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(['all', 'unread', 'read', 'trade', 'news', 'system', 'alert', 'signal'] as FilterType[]).map(filter => (
            <button
              key={filter}
            ...
            {filteredNotifications.map(notif => (
              <motion.li
                ...
                className={cn(
                  ...
                  notif.is_read ? "bg-transparent hover:bg-slate-700/20" : "bg-slate-700/10 hover:bg-slate-700/30"
                )}
              >
                {/* Type indicator */}
                <div className={cn(
                  ...
                  <span className="text-xs font-semibold uppercase">{notif.type?.slice(0,1)}</span>
                </div>
      ...
      {showConfirmClear && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm">
          ...
          >
            <h2 className="text-xl font-semibold text-white">Clear all notifications?</h2>
            <p className="mt-2 text-slate-400">This will delete all your notifications. This action cannot be undone.</p>
