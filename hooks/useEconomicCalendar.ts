...
import { getEconomicCalendar, determineImpact } from "@/lib/services/finnhubService";
  ...
  useEffect(() => {
    ...
    const fetchCalendarData = async () => {
      ...
      try {
        ...
        console.log(`[${requestId}] Fetching economic calendar for ${from} to ${to}`);
        ...
        if (isMounted) {
          setError(err instanceof Error ? err : new Error('Failed to fetch economic calendar'));
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
