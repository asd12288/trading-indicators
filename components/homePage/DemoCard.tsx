...
const DemoSignalCard: React.FC<DemoSignalCardProps> = ({ type }) => {
  ...
  return (
    <div className="flex min-h-screen scale-100 transform items-center justify-center lg:scale-110 xl:scale-125">
      {isRunning ? (
        <RunningSignalCard instrument={instrument} isBuy={isBuy} />
      ) : (
        <FufilledSignalCard instrument={instrument} isBuy={isBuy} />
      )}
    </div>
  );
};
