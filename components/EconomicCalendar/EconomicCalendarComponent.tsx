...
export default function EconomicCalendarComponent({ 
  ...
  useEffect(() => {
    ...
    const fetchEconomicEvents = async () => {
      try {
        ...
        const response = await fetch(`/api/economic-calendar?from=${today}&to=${endDate}`);
  ...
  const groupedEvents = events.reduce<Record<string, EconomicEvent[]>>((acc, event) => {
  ...
  if (error) {
    return <div className="text-center text-amber-500 p-8">{error}</div>;
  }

  if (!events || events.length === 0) {
    return <div className="text-center text-slate-400 p-8">No upcoming economic events found.</div>;
  }
          ...
          {Object.entries(groupedEvents).map(([date, dateEvents]) => (
            <React.Fragment key={date}>
              {dateEvents.map((event, index) => (
                <TableRow key={`${date}-${index}`} className={cn(
                  "transition-colors",
                  theme === "dark" 
                  ...
                  <TableCell className={tableCellClass}>{event.country}</TableCell>
                  <TableCell className={tableCellClass}>{event.indicator || event.period}</TableCell>
                  <TableCell className={tableCellClass}>{event.forecast ?? "N/A"}</TableCell>
                  <TableCell className={tableCellClass}>{event.actual ?? "N/A"}</TableCell>
                  <TableCell className={tableCellClass}>{event.previous ?? "N/A"}</TableCell>
                  <TableCell>
