...
window.TradeEventsBridge = {
  ...
  onTradeStart: (instrumentName, tradeSide, entryPrice) => {
    console.log(
      `[TradeEventsBridge] Trade started: ${instrumentName} ${tradeSide} at ${entryPrice}`,
    );
    tradeNotificationManager.notifyTradeStarted(
      instrumentName,
      tradeSide,
      entryPrice,
    );
  ...
  onTradeEnd: (instrumentName, tradeSide, exitPrice, profitLoss) => {
    console.log(
      `[TradeEventsBridge] Trade ended: ${instrumentName} ${tradeSide} at ${exitPrice} (P/L: ${profitLoss})`,
    );
    tradeNotificationManager.notifyTradeEnded(
      instrumentName,
      tradeSide,
      exitPrice,
      profitLoss,
  ...
  onTargetStopChange: (instrumentName, tradeSide, isStopLoss, newValue) => {
    console.log(
      `[TradeEventsBridge] ${isStopLoss ? "Stop-loss" : "Take-profit"} changed for ${instrumentName}: new value = ${newValue}`,
    );
    tradeNotificationManager.notifyTargetStopChanged(
      instrumentName,
      tradeSide,
      isStopLoss,
      newValue,
